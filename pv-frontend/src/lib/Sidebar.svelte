...
<script>
    ...
    onMount(() => {
        ...
        } else if (page === 'adminCompanies' || page === 'userCompanies') {
            const companiesIcon = document.querySelectorAll('#companies-icon');
            if (companiesIcon) {
                const paths = companiesIcon[0].querySelectorAll('path');
                paths.forEach(path => {
                    path.style.fill = 'white';
                });
            } 
        } else if (page === 'adminTemplates' || page === 'userTemplates') {
            const accountsIcon = document.querySelectorAll('#templates-icon');
            if (accountsIcon) {
                const paths = accountsIcon[0].querySelectorAll('path');
                paths.forEach(path => {
                    path.style.fill = 'white';
                });
            }
        } else if (page === 'userSmsService') {
            const smsServiceIcon = document.querySelectorAll('#sms-service-icon');
            if (smsServiceIcon) {
                const paths = smsServiceIcon[0].querySelectorAll('path');
                paths.forEach(path => {
                    path.style.fill = 'white';
                });
            }
    ...
    $: campaignsImageStyle = `
        ${page === 'adminCampaigns' ||page === 'userCampaigns' || page === 'userCampaigns2' ? 'color: white; font-size: 22px !important;' : ''}
        ${rotated ? 'margin-right: 0rem' : 'margin-right: 0.5rem'}`;
...
<div class="d-flex flex-column flex-shrink-0 justify-content-between" style="width: { sidebarWidth }px;" id="sidebar">
    ...
    <ul class="nav nav-pills flex-column p-4">
        ...
        <li>
            ...
            <div class="collapse d-flex justify-content-between" id="campaigns-collapse">
                <div class="vr mt-3 ms-2 me-2" style="background-color: #1783A8; width: 2px;"></div>
